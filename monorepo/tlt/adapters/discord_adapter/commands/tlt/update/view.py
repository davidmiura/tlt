...
class EventSelectView(discord.ui.View):
    def __init__(self, bot_instance: 'DiscordBot', events: list):
        ...
        if options:
            self.select_menu = discord.ui.Select(
                placeholder="✨ Pick your event to glow up... 🌟",
                options=options
            )
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        if event["creator_id"] != interaction.user.id and not self.bot_instance.is_admin(interaction.user):
            await interaction.response.send_message("🚫 Nah bestie, that's not your event to edit! Only the creator (or admin) can update this ✋😤", ephemeral=True)
            return
        
        # Show update modal
        from tlt.adapters.discord_adapter.commands.tlt.update.modal import EventUpdateModal
        modal = EventUpdateModal(self.bot_instance, message_id, event)
